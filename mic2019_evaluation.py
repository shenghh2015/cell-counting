		...
		if fnc_str == 'buildModel_Count_ception':
			...
			if type(preds) is list:
				...
				for i in range(preds[0].shape[0]):
					Y1 = np.squeeze(preds[0][i,:])/100
					Y2 = np.squeeze(preds[1][i,:])/100
					Y3 = np.squeeze(preds[2][i,:])/100
					Y4 = np.squeeze(preds[3][i,:])/100
					Yc = rescale(Y1,8)
# 					Yc = rescale(Y1,8) + rescale(Y2, 4.0) + rescale(Y3, 2.0) + Y4
					preds_list.append(Yc)
				for i in range(preds1[0].shape[0]):
					Y1 = np.squeeze(preds1[0][i,:])/100
					Y2 = np.squeeze(preds1[1][i,:])/100
					Y3 = np.squeeze(preds1[2][i,:])/100
					Y4 = np.squeeze(preds1[3][i,:])/100
					Yc = rescale(Y1,8)
# 					Yc = rescale(Y1,8) + rescale(Y2, 4.0) + rescale(Y3, 2.0) + Y4
					preds_list.append(Yc)
				for i in range(preds2[0].shape[0]):
					Y1 = np.squeeze(preds2[0][i,:])/100
					Y2 = np.squeeze(preds2[1][i,:])/100
					Y3 = np.squeeze(preds2[2][i,:])/100
					Y4 = np.squeeze(preds2[3][i,:])/100
					Yc = rescale(Y1,8)
# 					Yc = rescale(Y1,8) + rescale(Y2, 4.0) + rescale(Y3, 2.0) + Y4
					preds_list.append(Yc)
...
# 								np.concatenate([preds[2],preds1[2],preds2[2]], axis = 0), np.concatenate([preds[3],preds1[3],preds2[3]], axis = 0)]
				...
				preds_list = [np.concatenate([preds/100,preds1/100,preds2/100], axis = 0)]
# 		pred_list = []
...
# 	fnc_str = 'buildMultiModel_U_net'
	fnc_str = 'buildPrymaridModel_U_net'
